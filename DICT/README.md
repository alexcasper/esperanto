...
**27350 entries.** Every count in this section was measured against
...
| Corpus-mined, reviewed by hand | `corpus-mined` | 5120 |
...
4106 entries carry `derived: true`, marking a word built by regular affixation
...
**Corpus-mined** entries come from the 251-source corpus in `CORPUS/`: the
...
*samideano*) and productive derivations. All 5120 carry `attestation` (a
corpus `count` and the number of independent `sources`) and up to three real
`citations`; 3629 are attested in three or more independent sources.
...
12405 of them, including the cases where two reviewers disagreed.
